import React, { useEffect, useState } from "react"
import bg from "../../public/backimg.png"
import geyserImg from "../../public/1 (3).png"
import heaterImg from "../../public/1 (4).png"

const slides = [
    {
        title: "GEYSER",
        heading: "Smart Hot Water. Smarter Living.",
        image: geyserImg,
        leftTag: "Smart Temperature Control",
        rightTag: "Energy Efficient Technology",
        desc: "Control temperature, save energy, and enjoy comfort with next-gen smart geysers."
    },
    {
        title: "HEATER",
        heading: "Room Heaters That Care for Comfort",
        image: heaterImg,
        leftTag: "Auto Cut-Off Safety",
        rightTag: "Shockproof Design",
        desc: "Warmth that protects your family, saves energy, and lasts longer."
    }
]

const Home = () => {
    const [current, setCurrent] = useState(0)
    const [phase, setPhase] = useState("enter")

    useEffect(() => {
        const interval = setInterval(() => {
            setPhase("exit") // CENTER → RIGHT (exit)

            setTimeout(() => {
                setCurrent((prev) => (prev + 1) % slides.length)
                setPhase("enter") // LEFT → CENTER (enter)
            }, 700)
        }, 5000)

        return () => clearInterval(interval)
    }, [])

    const slide = slides[current]

    // 🎯 LEFT → CENTER → RIGHT Animation
    const slideAnim =
        phase === "enter"
            ? "translate-x-0 opacity-100"      // CENTER me stable
            : "translate-x-full opacity-0"     // RIGHT ko exit

    return (
        <section className="relative w-full min-h-[130svh] pb-40 overflow-hidden">

            {/* BACKGROUND */}
            <img
                src={bg}
                className="absolute inset-0 w-full h-full object-cover"
                alt="bg"
            />

            {/* DOTS */}
            <div className="absolute top-3 left-1/2 mt-20 -translate-x-1/2 z-20 flex gap-2">
                {slides.map((_, i) => (
                    <span
                        key={i}
                        className={`w-2.5 h-2.5 rounded-full transition
            ${current === i ? "bg-black" : "bg-black/30"}`}
                    />
                ))}
            </div>

            {/* WATERMARK */}
            <h1
                className="absolute top-40 md:top-10 left-1/2 -translate-x-1/2
        text-[60px] sm:text-[100px] md:text-[180px] lg:text-[220px] xl:text-[250px] 2xl:text-[300px]
        font-bold tracking-widest text-black/5 select-none transition-opacity duration-700"
                style={{ opacity: phase === "enter" ? 1 : 0 }}
            >
                {slide.title}
            </h1>

            {/* MAIN CONTENT */}
            <div className="relative z-10 max-w-7xl lg:mt-44 mx-auto px-4 h-full hidden md:block">
                <div className="grid grid-cols-1 md:grid-cols-3 items-center h-full gap-6 sm:gap-8 md:gap-10">

                    {/* LEFT CONTENT */}
                    <div className="order-2 md:order-1 text-center md:text-left">
                        <div
                            key={`left-${current}`}
                            className={`transition-all duration-700 ${slideAnim}`}
                            style={{
                                transform: phase === "enter" ? "translateX(0)" : "translateX(100%)",
                                animation: phase === "enter" ? "slideFromLeft 0.7s ease-out" : "none"
                            }}
                        >
                            <h4 className="font-semibold mb-4 text-sm sm:text-base md:text-lg lg:text-xl">
                                {slide.heading}
                            </h4>

                            <button className="px-5 py-2 sm:px-6 sm:py-3 bg-white rounded-full text-xs sm:text-sm md:text-base mb-8">
                                Shop Now
                            </button>

                            <div className="flex items-center gap-2 sm:gap-3 justify-center md:justify-start">
                                <span className="bg-white px-3 sm:px-4 py-1 border-2 border-amber-400 rounded-full text-[10px] sm:text-xs">
                                    {slide.leftTag}
                                </span>
                                <div className="w-10 sm:w-16 h-[2px] bg-amber-400" />
                            </div>
                        </div>
                    </div>

                    {/* CENTER IMAGE */}
                    <div className="order-1 md:order-2 relative flex justify-center items-center h-[420px] sm:h-[520px] md:h-[620px] lg:h-[820px] xl:h-[760px]">
                        <img
                            key={`img-${current}`}
                            src={slide.image}
                            alt="product"
                            className={`
                                absolute
                                transition-all duration-700 ease-in-out
                                ${slideAnim}
                                object-contain
                                ${current === 1
                                    ? 'w-[220px] sm:w-[300px] md:w-[420px] lg:w-[760px] xl:w-[640px] 2xl:w-[1100px]'
                                    : 'w-[180px] sm:w-[240px] md:w-[320px] lg:w-[720px] xl:w-[480px] 2xl:w-[620px]'
                                }
                            `}
                            style={{
                                transform: phase === "enter" ? "translateX(0)" : "translateX(100%)",
                                animation: phase === "enter" ? "slideFromLeft 0.7s ease-out" : "none"
                            }}
                        />
                    </div>

                    {/* RIGHT CONTENT */}
                    <div className="order-3 text-center md:text-right">
                        <div
                            key={`right-${current}`}
                            className={`transition-all duration-700 ${slideAnim}`}
                            style={{
                                transform: phase === "enter" ? "translateX(0)" : "translateX(100%)",
                                animation: phase === "enter" ? "slideFromLeft 0.7s ease-out" : "none"
                            }}
                        >
                            <div className="flex items-center gap-2 sm:gap-3 justify-center md:justify-end mb-4 sm:mb-6">
                                <div className="w-10 sm:w-16 h-[2px] bg-amber-400" />
                                <span className="bg-white px-3 sm:px-4 py-1 border-2 border-amber-400 rounded-full text-[10px] sm:text-xs">
                                    {slide.rightTag}
                                </span>
                            </div>

                            <p className="text-xs sm:text-sm md:text-base lg:text-lg font-semibold text-gray-700 leading-relaxed">
                                {slide.desc}
                            </p>
                        </div>
                    </div>

                </div>
            </div>

            {/* ================= MOBILE CAROUSEL ================= */}
            <div className="md:hidden relative z-10 px-4 mt-64 text-center">

                {/* TEXT */}
                <div
                    key={`mobile-text-${current}`}
                    className="mt-6 transition-all duration-700"
                    style={{
                        animation: phase === "enter" ? "slideFromLeft 0.7s ease-out" : "none"
                    }}
                >
                    <h4 className="font-semibold text-base mb-3">
                        {slide.heading}
                    </h4>

                    <p className="text-sm text-gray-700 leading-relaxed px-4">
                        {slide.desc}
                    </p>
                </div>
                {/* BUTTON */}
                <div className="mt-6 flex justify-center">
                    <button className="px-6 py-3 w-48 bg-white text-black rounded-full text-sm border shadow-md">
                        Shop Now
                    </button>
                </div>
                {/* IMAGE */}
                <div className="h-[460px] flex items-center justify-center overflow-hidden">
                    <img
                        key={`mobile-img-${current}`}
                        src={slide.image}
                        alt="product"
                        className={`
        transition-all duration-700 ease-in-out
        object-contain
        w-[320px]
        h-[820px]
        ${phase === "enter" ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"}
      `}
                        style={{
                            animation: phase === "enter" ? "slideFromLeft 0.7s ease-out" : "none"
                        }}
                    />
                </div>




            </div>


            {/* CSS Animation */}
            <style>{`
                @keyframes slideFromLeft {
                    from {
                        transform: translateX(-100%);
                        opacity: 0;
                    }
                    to {
                        transform: translateX(0);
                        opacity: 1;
                    }
                }
            `}</style>

        </section>
    )
}

export default Home
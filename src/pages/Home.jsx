...
const Home = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            // Step 1: current slide exits left
            setPhase("left");

            // Step 2: after exit animation, move to next slide
            setTimeout(() => {
                setCurrent((prev) => (prev + 1) % slides.length);
                setPhase("enter"); // next slide enters from right
            }, 700); // animation duration
        }, 5000); // slide every 5s
    ...
    return (
        ...
        font-bold tracking-widest text-black/5 select-none"
            ...
            {/* MAIN CONTENT */}
            <div className="relative z-10 max-w-7xl lg:mt-44 mx-auto px-4 h-full">
                <div className="grid grid-cols-1 md:grid-cols-3 items-center h-full gap-6 sm:gap-8 md:gap-10">
    ...
    flex justify-center 
    items-center
    h-[420px] sm:h-[520px] md:h-[620px] lg:h-[820px] xl:h-[760px]
  "
                    >
                        <img
                            src={slide.image}
